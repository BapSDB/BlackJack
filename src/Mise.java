...
public class Mise
{
    ...
    public static class Builder {
        ...
        private Builder() throws IOException
        {
            ...
            while (jetons == 0) {
                System.out.print("Nombre de jetons initial ? ");
                try {
                    jetons = Integer.parseUnsignedInt(buffer.readLine());
                } catch (NumberFormatException ignored) {
                }
            }
            ...
            for (int i = 1; i <= nbJoueurs; i++) {
                ...
                noms.add(nom);
                joueurs.add(Joueur.nouveau(Joueur.Type.HUMAIN, nom, jetons));
            }
        }
    ...
    private static Mise mise;

    public static Mise getInstance() {
        if (mise == null) {
            try {
                mise = new Mise.Builder().getMise();
            } catch (IOException e) {
    ...
    private void premierTour() throws IOException {
        final List<Joueur> joueurs = mise.joueursEnLice;
        final List<List<Joueur>> classement = mise.classement;
        ...
        IntStream.range(0, mise.nbJoueurs)
    ...
    private void tourJoueur() throws IOException {
        final List<Joueur> joueurs = mise.joueursEnLice;
        final Joueur joueur = joueurs.get(mise.joueurCourant);
        final String nom = joueur.getNom();
        final int score = joueur.getScore();
        final List<List<Joueur>> classement = mise.classement;
        ...
        if (!continuer) {
            joueurs.remove(mise.joueurCourant);
    ...
    private void tourComplet() throws IOException {
        mise.nbTours++;
        System.out.println("\n*******************************\n");
        System.out.println("TOUR " + mise.nbTours + " :");
        ...
        mise.joueurCourant = 0;
        final int nombreTours = mise.joueursEnLice.size();
        for (int i = 0; i < nombreTours; i++) tourJoueur();
    }

    private boolean partieFinie() {
        return mise.joueursEnLice.isEmpty();
    }

    private void partieComplete() throws IOException {

        // On affiche les mises de chaque joueur, le nombre de jetons restants de chacun
        // et la some promise au vainqueur

        joueursEnLice.forEach(joueur -> {
            System.out.println(joueur.getNom() + " a misé " + joueur.getMise() + " jetons.");
            System.out.println("Il lui reste " + joueur.getJetons() + " jeton(s).");
            System.out.println();
        });

        System.out.println("Le vainqueur de la partie remportera " + jetonsEnJeu + " jetons.");
        System.out.println();
        ...
        for (List<Joueur> joueurs : mise.classement)
            if (!joueurs.isEmpty())
                joueurs.stream()
        ...
        // On ajoute la mise en jeu au nombre de jetons du vainqueur
        // On ré-initialise les joueurs toujours en lice pour le prochain tour
        premier.ajouterJetons(jetonsEnJeu);
        final List<Joueur> joueursRestants = mise.joueurs.stream().
                filter(joueur -> joueur.getJetons() > 0)
                .toList();
        nbJoueursRestants = joueursRestants.size();
        joueursEnLice.addAll(joueursRestants);
        ...
        // On affiche la liste des joueurs et le nombre de jetons qu'il leur reste
        // Du plus petit nombre au plus grand
        System.out.println("Le classement actuel selon le nombre de jetons restants est : ");
        mise.joueurs.stream()
                .sorted(Comparator.comparing(Joueur::getJetons))
                .map(joueur -> joueur.getNom() + " avec " + joueur.getJetons() + " jeton(s)")
                .forEach(System.out::println);
        System.out.println();
        ...
        final Map<Integer, List<String>> classement = mise.joueurs.stream()
        ...
        final int nbParties = mise.nbParties;
        System.out.println((n > 1 ? "Les" : "Le") +
                (n > 1 ? " joueurs " : " joueur ") +
    ...
    public void jouer() throws IOException {
        ...
        while (console.getRegexReponse().test(reponse)) {
            mise.nbParties++;
            System.out.println("\n*******************************\n");
            System.out.println("PARTIE " + mise.nbParties + " :");
            premierTour();
            partieComplete();
        ...
        meilleurJoueur = mise.joueurs.stream().filter(joueur -> joueur.getJetons() > 0).findAny();
        if (nbJoueursRestants <= 1 && meilleurJoueur.isPresent()) {
